...
BEGIN
  ...
  INTO
    test_runs, test_dbs, start_times, end_times, last_times
  FROM workload_test.test_run_configurations
  WHERE now() > start_time
    AND (last_copy_time IS NULL OR last_copy_time < end_time);
  ...
  WHILE i <= cfg_count LOOP
    ...
    WHERE e.database_name = test_db
      AND e.collection_ts BETWEEN from_ts AND to_ts;
    ...
    WHERE i.database_name = test_db
      AND i.start_time BETWEEN from_ts AND to_ts;

    -- 5) insert statement stats
    ...
    WHERE s.aggregated_ts
          BETWEEN date_trunc('hour', from_ts)
              AND date_trunc('hour', to_ts)
    ON CONFLICT (test_run, aggregated_ts, fingerprint_id, transaction_fingerprint_id, app_name)
    DO UPDATE SET
    ...
    -- 7) advance last_copy_time so we don’t double-copy next run
    UPDATE workload_test.test_run_configurations
    SET last_copy_time = to_ts
    WHERE test_run = test_name;

    -- 8) now compute counts by querying each target table
    ...
    -- 9) collect this run’s counts into our JSONB array
